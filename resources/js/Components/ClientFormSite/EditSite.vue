...
export default{
  ...
  data(){
    return {
      document : this.site.document,
      fileIncrement : this.site.document.length,
    }
  },
  setup(props){
    const form = useForm({
                ...
                <div v-if="document.length" class="grid grid-cols-1 mb-5 mx-7">
                  ...
                  <dd class="mt-1 text-sm text-gray-900 col-span-6 sm:col-span-6">
                    <ul role="list" class="divide-y divide-gray-200 rounded-md">
                      <li v-for="(file,index) in site.document"  class="pl-3 pr-4 py-3 flex-col items-center justify-center text-sm">
