      ...
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 sm:block sm:p-0 ">
        <div class="fixed inset-0 " aria-hidden="true">
          <div class="absolute inset-0 bg-gray-500 opacity-50" ></div>
            ...
            <hr/>
            <form ref="addsiteform" @submit.prevent="submitForm">
            ...
            <div class="grid grid-cols-1 mt-5 mx-7">
              ...
              <textarea type="text" v-model="form.observation" ref="inputs" id="observation" autocomplete="off" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-sm" ></textarea>
            </div>

            <Uploadfile class="grid grid-cols-1 my-5 mx-7" @input="uploadMedia" ></Uploadfile>
            ...
            <div class='flex items-center justify-end  md:gap-8 gap-4 pt-5 pb-5 mr-7'>
              <button type="button" @click="close" class='inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium cursor-pointer rounded-md text-white bg-gray-500 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500'>Annuler</button>
    ...
    close() {
      ...
      if(this.document.length){
        for(let i=0;i<this.document.length;i++){
          ...
          else if(this.document[i].source && !this.document[i].erreur){
            k++;
            splice.push({
              item : this.document[i],
              index : i
      ...
      else{this.$refs.addsiteform.reset();
          this.document=[]
          this.$emit('close')}
      }
      else{this.$refs.addsiteform.reset();
