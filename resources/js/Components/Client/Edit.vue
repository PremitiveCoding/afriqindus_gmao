        ...
        <form @submit.prevent="updateClient">
          <div class="shadow overflow-hidden sm:rounded-md">
            <div class="px-4 py-5 bg-white sm:p-6">
              <div class="grid grid-cols-6 gap-6">
                ...
                <div class="col-span-6 sm:col-span-6 lg:col-span-3">
                  <label for="tel" class="block text-sm font-medium text-gray-700">Télephone</label>
            ...
            <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
              <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" :class="{'cursor-not-allowed bg-indigo-500 hover:bg-indigo-500' : isloading}" :disabled="isloading">Enregistrer</button>
            </div>
          </div>
        </form>
      </div>
...
export default{
  ...
  setup(props){
    const form = useForm({
      ...
      ntva:  props.client.ntva,
      email:  props.client.email,
      emailSs: props.client.emailSs,
      emailFact: props.client.emailFact,
      adresse: props.client.adresse,
      adresseSs: props.client.adresseSs,
      tele: props.client.tele,
      observation: props.client.observation,
      siteInternet: props.client.siteInternet,
    });
    return{form}
  },
  computed :{
  ...
  methods: {
    ...
    Inertia.post(  `/clients/${this.client.id}`,{
      _method:'put',
      nom: this.form.nom ,
      prenom: this.form.prenom,
      ntva:  this.form.ntva, 
      email:  this.form.email,
      emailSs: this.form.emailSs,
      emailFact: this.form.emailFact,
      adresse: this.form.adresse,
      adresseSs: this.form.adresseSs,
      tele: this.form.tele,
      observation: this.form.observation,
      siteInternet: this.form.siteInternet,
  ...
  mounted() {
    let item = []
    this.fileIncrement=this.client.documents.length
    if(this.client.documents.length){
        for(let i=0;i<this.client.documents.length;i++){
          item.push({
            id : this.client.documents[i].id,
            name : this.client.documents[i].title,
            type : this.client.documents[i].type,
            size : this.client.documents[i].size,
            source : undefined,
            imported : true,
            progress : 100,
            loading : false,
